...
class AccountPartnerOpenItemsXlsx(models.AbstractModel):
    ...
    def _get_ws_params(self, workbook, data, partners):  # noqa: C901
        ...
        CASE WHEN l.balance > 0
          THEN (
            SELECT COALESCE(SUM(pr.debit_amount_currency), 0)
            FROM account_partial_reconcile pr
            INNER JOIN account_move_line prl ON pr.credit_move_id = prl.id
            WHERE pr.debit_move_id = l.id
            AND l.currency_id = pr.debit_currency_id
            AND prl.date <= '{wiz.date_at}'
          )
          ELSE (
            SELECT COALESCE(SUM(-pr.credit_amount_currency), 0)
            FROM account_partial_reconcile pr
        ...
        query_start = (
            ...
            "l.full_reconcile_id, l.matching_number AS matching_number, "
    ...
    AND (
      (l.full_reconcile_id IS NULL AND
        ...
        if wiz.result_selection == "asset_receivable":
            reports = [report_ar]
        elif wiz.result_selection == "liability_payable":
            reports = [report_ap]
        elif wiz.result_selection == "all":
            ...
            if lines:
                ...
                for p in p_map:
                    ...
                    if p["p_id"] not in [x.get("p_id", None) for x in partners]:
                        ...
                        if not p["p_id"]:
                            p_full_name = _("No partner allocated")
                        else:
                            p_full_name = p["p_name"] or _("n/a")
                            if p["p_ref"]:
        ...
        for report in reports:
            ...
            ws_params_details = {
                "ws_name": report["title_short"] + " " + _("Details"),
    ...
    def _get_report_title(self, data, wiz, report, report_type):
        ...
        if wiz.target_move == "posted":
            title += s + _("All Posted Entries")
        else:
            title += s + _("All Entries")
    ...
    def _empty_report(self, workbook, ws, ws_params, data, partners):
        ...
        no_items = _("No Open Items found for your selection")
        ws.write_string(row_pos, 0, no_items, FORMATS["format_left_bold"])
    ...
    def _get_overview_template(self):
        template = {
            ...
            "partner_ref": {
                "header": {"type": "string", "value": _("Partner Reference")},
    ...
    def _get_details_template(self):
        template = {
            "document": {
                "header": {"type": "string", "value": _("Document")},
            ...
            "sup_inv_nr": {
                "header": {"type": "string", "value": _("Supplier Invoice No")},
                "lines": {
                    "type": "string",
            ...
            "origin": {
                "header": {"type": "string", "value": _("Source Document")},
                "lines": {
                    "type": "string",
    ...
    def _details_report(self, workbook, ws, ws_params, data, partners):
        ...
        for partner in report["partners"]:
            ws.write_string(
                row_pos, 0, partner["p_full_name"], FORMATS["format_left_bold"]
            )
            row_pos += 1
            row_pos = self._write_line(
                ws,
            ...
            for line in partner["lines"]:
                ...
                if wiz.add_reconcile:
                    ...
                    if line["matching_number"]:
                        rec_details += " (%s)" % line["matching_number"]
