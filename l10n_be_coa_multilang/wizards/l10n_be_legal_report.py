...
class L10nBeLegalReport(models.TransientModel):
    ...
    type = fields.Selection(
        selection=[("bs", "Balance Sheet"), ("pl", "Profit & Loss")]
    )
    ...
    def _onchange_company_id(self):
        ...
        if self.company_id:
            dates = self.company_id.compute_fiscalyear_dates(fields.Date.today())
...
class L10nBeLegalReportLine(models.TransientModel):
    ...
    sequence = fields.Integer(related="chart_id.sequence", store=True)
    level = fields.Integer()
    active = fields.Boolean()
    color = fields.Char(related="chart_id.color")
    font = fields.Selection(related="chart_id.font")
    ...
    def view_move_lines(self):
        ...
        if self.chart_id.child_ids:
            ...
            charts = self.chart_id.search(
                [("parent_id", "child_of", self.chart_id.id), ("child_ids", "=", False)]
            )
...
class L10nBeLegalReportXlsx(models.AbstractModel):
    ...
    def _get_ws_params(self, workbook, data, be_report):
        ...
        col_specs = {
            "code": {
                ...
                "lines": {"value": self._render("l.chart_id.code")},
                "width": 8,
            },
            "name": {
                "header": {"value": self._("Description")},
                "lines": {"value": self._render("l.chart_id.name")},
                "width": 70,
            },
            "amount": {
                "header": {
    ...
    def _generate_be_report(self, workbook, ws, ws_params, data, be_report):
        ...
        row_pos = 0
        row_pos = self._be_report_title(ws, row_pos, ws_params, data, be_report)
        row_pos = self._be_report_info(ws, row_pos, ws_params, data, be_report)
        row_pos = self._be_report_lines(ws, row_pos, ws_params, data, be_report)
    ...
    def _be_report_info(self, ws, row_pos, ws_params, data, be_report):
        ...
        if be_report.type == "pl":
            ws.write_string(
                row_pos, 1, self._("Start Date") + ":", FORMATS["format_left_bold"]
            )
            date_from = fields.Date.to_string(be_report.date_from)
            ws.write_string(row_pos, 2, date_from)
            row_pos += 1
        ws.write_string(
            row_pos, 1, self._("End Date") + ":", FORMATS["format_left_bold"]
        )
