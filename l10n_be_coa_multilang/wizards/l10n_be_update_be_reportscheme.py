...
class L10nBeUpdateBeReportscheme(models.TransientModel):
    _name = "l10n.be.update.be.reportscheme"
    _description = "Update BNB/NBB financial reports configuration"

    update_account_type = fields.Boolean(help="Update also Account Types")
    update_account_tags = fields.Boolean(help="Update also Account Tags")
    ...
    def _update_be_reportscheme(self, company):
        ...
        for account in accounts:
            if account == company.account_journal_suspense_account_id:
                # no warning for the Bank Suspense account
                continue
            if account.account_type == "equity_unaffected":
                # no warning for the Undistributed Profit/Losses account
                continue
        ...
        if non_be_scheme_accounts:
            non_be_scheme_accounts = non_be_scheme_accounts.sorted(lambda r: r.code)
            note += _(
                "Following accounts are not included in "
                "the legal Belgian Balance and P&L reports:\n\n"
            )
            for acc in non_be_scheme_accounts:
                note += "Code: {} (id: {}), company: {}\n".format(
