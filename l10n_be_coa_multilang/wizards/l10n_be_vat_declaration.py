...
class L10nBeVatDeclaration(models.TransientModel):
    ...
    def _get_case_domain(self, case_report):
        if case_report.children_line_ids:
            ...
            for i, case in enumerate(cases, start=1):
                if i != len(cases):
                    case_dom += ["|"]
                case_dom += [("tax_tag_ids", "in", case.tag_ids.ids)]
        else:
            case_dom = [("tax_tag_ids", "in", case_report.tag_ids.ids)]
        return case_dom
    ...
    def _get_case_amounts(self, case_root, tax_code_map):
        ...
        dom_min = [
            "|",
            ("move_id.move_type", "in", min_types),
            "&",
        ...
        dom_plus = [
            "|",
            ("move_id.move_type", "not in", min_types + ["entry"]),
            "&",
            ("move_id.move_type", "=", "entry"),
            ("balance", ">", 0),
        ]
...
class L10nBeVatDeclarationXlsx(models.AbstractModel):
    ...
    def _declaration_info(self, ws, row_pos, ws_params, data, declaration):
        ...
        row_pos += 1
        ws.write_string(row_pos, 1, self._("Period") + ":", FORMATS["format_left_bold"])
    ...
    def _journal_lines(self, ws, row_pos, ws_params, data, decl, journal):
        ...
        for aml in amls:
            ...
            for tag in aml.tax_tag_ids:
                ...
                sign = tag.tax_negate and -1 or 1
                if am.move_type in min_types or (
                    am.move_type == "entry" and aml.balance < 0
                ):
                    sign = sign * -1
