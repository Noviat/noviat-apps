...
class L10nBeVatCommon(models.AbstractModel):
    ...
    year = fields.Char(size=4, required=True, default=lambda self: self._default_year())
    ...
    target_move = fields.Selection(
        selection=[("posted", "Posted Entries"), ("all", "All Entries")],
        string="Target Moves",
        required=True,
        default="posted",
    )
    ...
    def view_draft_moves(self):
        ...
        act = clean_action(act, env=self.env)
        act["domain"] = dom
        act["context"] = {}
        return act
    ...
    def create_xml(self):
        raise UserError(_("The XML export function is not available."))
    ...
    def _get_company_data(self):
        ...
        if not company_street:
            raise UserError(_("No 'Street' for %s") % cpart.name)
        if cpart.street2:
            company_street += " " + cpart.street2

        company_zip = cpart.zip
        if not company_zip:
            raise UserError(_("No 'ZIP Code' for %s") % cpart.name)

        company_city = cpart.city
        if not company_city:
            raise UserError(_("No 'City' for %s") % cpart.name)
    ...
    def _get_declarant_data(self):
        ...
        if not email:
            raise UserError(_("No 'email' for %s") % self.declarant_id.name)

        phone = self.declarant_id.phone
        if not phone:
            raise UserError(_("No 'phone' for %s") % self.declarant_id.name)
        phone = phone.replace("+", "00").replace(" ", "")
        no_break_space = "\u00A0"
        phone = phone.replace(no_break_space, "")

        declarant_data = {"email": email, "phone": phone}
    ...
    def _node_Representative(self, parent, ns_map):
        ...
        Representative = etree.SubElement(parent, "Representative")
    ...
    def _action_save_xml(self):
        module = __name__.split("addons.")[1].split(".")[0]
        result_view = self.env.ref("%s.l10n_be_vat_common_view_form_xml" % module)
