...
class L10nBeVatListing(models.TransientModel):
    ...
    def create_xml(self):
        ...
        Doc = etree.Element(
            "ClientListingConsignment", attrib={"ClientListingsNbr": "1"}, nsmap=ns_map
        )
    ...
    def print_declaration(self):
        module = __name__.split("addons.")[1].split(".")[0]
        return self.env.ref("%s.action_report_l10nbevatlisting" % module).report_action(
            self
        )
    ...
    def _get_move_line_tax_domains(self):
        ...
        aml_dom_base = [("tax_tag_ids.id", "in", tags_base.ids)]
        ...
        aml_dom_vat = [("tax_tag_ids.id", "in", tags_vat.ids)]

        return aml_dom_base, aml_dom_vat
    ...
    def _node_ClientListing(self, parent, ns_map, ref):
        ...
        TurnOverSum = sum(x["base_amount"] for x in list(client_list.values()))
        VATAmountSum = sum(x["vat_amount"] for x in list(client_list.values()))
    ...
    def _node_Client(self, ClientListing, ns_map, client, seq):
        Client = etree.SubElement(
            ClientListing, "Client", attrib={"SequenceNumber": str(seq)}
        )

        CompanyVATNumber = etree.SubElement(
            Client, "CompanyVATNumber", attrib={"issuedBy": client["vat"][:2]}
        )
        CompanyVATNumber.text = client["vat"][2:]

        TurnOver = etree.SubElement(Client, "TurnOver")
        TurnOver.text = "%.2f" % client["base_amount"]

        VATAmount = etree.SubElement(Client, "VATAmount")
...
class L10nBeVatListingClient(models.TransientModel):
    ...
    listing_id = fields.Many2one(
        comodel_name="l10n.be.vat.listing", string="Client Listing"
    )
    partner_id = fields.Many2one(comodel_name="res.partner", readonly=1)
    vat = fields.Char(string="VAT Number", readonly=1)
    base_amount = fields.Monetary(currency_field="currency_id", readonly=1)
    vat_amount = fields.Monetary(
        string="VAT Amount", currency_field="currency_id", readonly=1
    )
    currency_id = fields.Many2one(related="listing_id.currency_id", readonly=1)
...
class L10nBeVatListingXlsx(models.AbstractModel):
    ...
    def _listing_info(self, ws, row_pos, ws_params, data, listing):
        ws.write_string(
            row_pos, 1, self._("Company") + ":", FORMATS["format_left_bold"]
        )
        ws.write_string(row_pos, 2, listing.company_id.name)
        row_pos += 1
        ws.write_string(
            row_pos, 1, self._("VAT Number") + ":", FORMATS["format_left_bold"]
        )
        ws.write_string(row_pos, 2, listing.company_id.vat or "")
        row_pos += 1
        ws.write_string(row_pos, 1, self._("Year") + ":", FORMATS["format_left_bold"])
        ws.write_string(row_pos, 2, listing.year)
        row_pos += 1
        ws.write_string(
            row_pos, 1, self._("Target Moves") + ":", FORMATS["format_left_bold"]
        )
        ws.write_string(
            row_pos,
            2,
