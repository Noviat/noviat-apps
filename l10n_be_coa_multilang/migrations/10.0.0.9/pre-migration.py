...
def remove_data(cr, module, model):
    ...
    """
    cr.execute(
        "SELECT id, res_id FROM ir_model_data WHERE module=%s AND model=%s",
        (module, model),
    )
    res = cr.fetchall()
    if res:
...
def update_ir_model_data(cr, module, model):
    ...
    if res:
        ...
        cr.execute(
            "UPDATE ir_model_data SET noupdate = False WHERE id IN %s", (res_ids,)
        )
...
def migrate(cr, version):
    ...
    cr.execute(
        "SELECT res_id FROM ir_model_data WHERE module=%s AND model=%s", (module, model)
    )
    res = cr.fetchall()
    ...
    cr.execute(
        "DELETE FROM be_legal_financial_reportscheme WHERE id IN %s",
        (tuple(scheme_ids),),
    )
    ...
    cr.execute(
        "SELECT res_id FROM ir_model_data WHERE module=%s AND model=%s AND name=%s",
        (module, model, name),
    )
    res = cr.fetchone()
