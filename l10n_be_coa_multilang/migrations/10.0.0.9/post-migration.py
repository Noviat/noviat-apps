...
def migrate(cr, version):
    ...
    for at_id, at_code, at_name in at_codes:
        ...
        if res_id:
            cr.execute(
                "SELECT att.name FROM account_tax_template att WHERE id = %s",
                (res_id[0],),
            )
        ...
        if att_name != at_name:
            ...
            try:
                with cr.savepoint():
                    cr.execute(
                        "UPDATE account_tax SET name = %s WHERE id = %s",
                        (att_name, at_id),
                    )
            except Exception:
                _logger.error(
                    "Update of account.tax,%s with name '%s' failed.", at_id, att_name
                )
