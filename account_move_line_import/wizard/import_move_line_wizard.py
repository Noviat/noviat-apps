...
class AccountMoveLineImport(models.TransientModel):
    ...
    dialect = fields.Char(
        help='JSON dictionary to store the csv dialect')
    ...
    def _read_csv(self, data):
        ...
        lines, header = self._remove_leading_lines(
            lines.decode(self.codepage))
        dialect_dict = json.loads(self.dialect)
        header_fields = next(csv.reader(
            io.StringIO(header), **dialect_dict))
            ...
            if not header:
                ...
                for ci, hf in enumerate(self._header_fields):
                    ...
                    if fmt == 'char':
                        ...
                        elif cell.ctype in [xlrd.XL_CELL_NUMBER,
                                            xlrd.XL_CELL_DATE]:
                            val = xlrd.xldate.xldate_as_tuple(
    ...
    def _input_fields(self):
        ...
        res = {
            ...
            'debit': {'method': self._handle_debit,
                      'field_type': 'float'},
            'credit': {'method': self._handle_credit,
                       'field_type': 'float'},
    ...
    def _log_line_error(self, line, msg):
        if not line.get('log_line_error'):
            data = ' | '.join(
                ['%s' % line[hf] for hf in self._header_fields])
            self._err_log += _(
        ...
        if not aml_vals.get(orm_field):
            ...
            if val:
                if isinstance(val, str):
                    val = str2int(
                        val.strip(), self.decimal_separator)
...
def dialect2dict(dialect):
    attrs = ['delimiter', 'doublequote', 'escapechar', 'lineterminator',
