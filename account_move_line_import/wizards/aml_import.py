...
class AmlImport(models.TransientModel):
    ...
    def _get_orm_fields(self, wiz_dict):
        ...
        blacklist = ["move_id"] + models.MAGIC_COLUMNS + AML_IMPORT_BLACKLIST
        wiz_dict["orm_fields"] = {
            f: orm_fields[f]
            for f in orm_fields
    ...
    def _process_header(self, wiz_dict, header_fields):
        ...
        for hf in header_fields:
            ...
            if hf not in wiz_dict["orm_fields"] and hf not in [
                ...
                _logger.error(
                    _(
                        "%(name)s, unsupported field '%(hf)s' found while importing "
                        "move lines"
                    )
    ...
    def _handle_orm_integer(
        ...
        if not aml_vals.get(orm_field):
            ...
            if val:
                ...
                if val is False:
                    msg = _(
                        "Incorrect value '%(val)s' for field '%(field)s' of "
                        "type Integer !"
                    ) % {
                        "val": line[field],
                        "field": field,
    ...
    def _handle_orm_json(self, wiz_dict, field, line, move, aml_vals, orm_field=False):
        raise NotImplementedError
