...
class AccountCodaImport(models.TransientModel):
    ...
    def _prepare_new_aml_dict(self, wiz_dict, st_line, cba, transaction):
        ...
        # the st_line reconcile method assumes that the
        # input mv_line_dict 'balance' contains the amount
        # in bank statement line currency and will handle the currency
        # conversions
        new_aml_dict["balance"] = -transaction["amount"]
        return new_aml_dict
    ...
    def _prepare_counterpart_aml_dicts(self, wiz_dict, st_line, cba, transaction):
        ...
        for entry in match["counterpart_amls"]:
            ...
            if (
                ...
                # conversions
                balance = -entry[1]
            counterpart_aml_dict["balance"] = balance
    ...
    def _create_move_and_reconcile(
        ...
        if match.get("counterpart_amls"):
            lines_vals_list += self._prepare_counterpart_aml_dicts(
                wiz_dict, st_line, cba, transaction
            )
        ...
        if lines_vals_list:
            if match.get("partner_id") and st_line.partner_id.id != match["partner_id"]:
