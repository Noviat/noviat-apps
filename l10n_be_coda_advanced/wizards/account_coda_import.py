...
class AccountCodaImport(models.TransientModel):
    ...
    # TODO: restore default=True after porting the automatic reconcile code
    ...
    def _coda_parsing(self, wiz_dict, codafile, codafilename):  # noqa: C901
        ...
        for coda_statement in coda_statements:
            ...
            wiz_dict["coda_import_note"] = wiz_dict["coda_import_note"] + _(
                "\n\nBank Journal: %s"
                "\nCODA Version: %s"
    ...
    def _automatic_reconcile(
        ...
        self.accounting_date = self.statement.accounting_date
        ...
        for st_line_ids in splitted_line_ids:
            reconcile_note, error = self._st_line_reconcile_thread(
                wiz_dict, st_line_ids, reconcile_note
            )
            if error:
                break
    ...
    def _st_line_reconcile_thread(self, wiz_dict, st_line_ids, reconcile_note):
        error = False
    ...
    SELECT id FROM (
      SELECT id, type, state, amount_total, name, invoice_payment_ref, ref,
             '{}'::text AS free_comm
        FROM account_move
        WHERE invoice_payment_state != 'paid' AND company_id = {} AND state = 'posted'
      ) sq
    ...
    def _match_aml_arap_refine(self, wiz_dict, st_line, cba, transaction, matches):
        """
    ...
    def _match_aml_arap(self, wiz_dict, st_line, cba, transaction, reconcile_note):
        ...
        matches = self._match_aml_arap_refine(
            wiz_dict, st_line, cba, transaction, matches
        )
    ...
    def get_bank(self, wiz_dict, bic, iban):
